...
export function EvaluationWorkspace({ role }: { role: RoleId }) {
  ...
  return <><div className="page-heading"><div><h1>Evaluations</h1><p>{student ? 'View and download your finalized internship reports.' : 'Evaluate assigned interns and track coordinator review.'}</p></div>{!student && template && assignments.length > 0 && <button className="button button-primary" onClick={() => setEditor(null)}><Plus size={18} />Evaluate intern</button>}</div>
    ...
    {loading ? <div className="card page-skeleton" role="status" aria-label="Loading evaluations"><span /><span /><span /></div> : !records.length ? <section className="card empty-action"><h2>{student ? 'No finalized evaluations yet' : 'No evaluations yet'}</h2><p>{student ? 'Your report will appear after coordinator finalization.' : 'Evaluations for your authorized assignments will appear here.'}</p></section> : <section className="card table-card"><div className="table-scroll"><table className="data-table"><thead><tr><th>Student</th><th>Evaluation</th><th>Evaluator</th><th>Status</th><th>Actions</th></tr></thead><tbody>{records.map(r => <tr key={r.id}><td>{r.studentName}</td><td>{r.templateName}</td><td>{r.evaluatorName}</td><td>{r.status}</td><td><div className="inline-actions"><button className="table-link" onClick={() => setDetails(r.id)}><Eye size={16} />View details</button><button className="table-link" onClick={() => setSelected(r.id)}><Eye size={16} />View report</button>{!student && r.evaluatorId === user?.id && ['Draft','Returned'].includes(r.status) && <button className="table-link" onClick={() => setEditor(r)}>Edit</button>}{!student && r.evaluatorId === user?.id && r.status === 'Draft' && <button className="table-link" title="Delete your draft" onClick={() => setAction({ record: r, kind: 'delete' })}><Trash2 size={16} />Delete</button>}</div></td></tr>)}</tbody></table></div></section>}
    {detail && <section className="card evaluation-report"><header className="official-form-heading"><span className="evaluation-report-kicker">Performance evaluation for SIE/SJT/OJT students</span><h2>{detail.studentName}</h2><p>{detail.status} · {detail.metadata?.formCode ?? detail.templateName}</p></header><dl className="report-identity"><div><dt>Student</dt><dd>{detail.studentName}</dd></div><div><dt>Status</dt><dd>{detail.status}</dd></div><div><dt>Form</dt><dd>{detail.metadata?.formCode ?? detail.templateName}</dd></div><div><dt>Rating period</dt><dd>{detail.context.ratingPeriod || 'Not recorded'}</dd></div><div><dt>Rater</dt><dd>{detail.evaluatorName}</dd></div><div><dt>Designation / office</dt><dd>{[detail.context.designation, detail.context.office].filter(Boolean).join(' · ') || 'Not recorded'}</dd></div></dl><RatingSections criteria={detail.criteria} metadata={detail.metadata} /><section className="report-remarks"><h3>Remarks</h3><p className="preserve-text">{detail.overallRemarks || 'No remarks recorded.'}</p></section>{detail.status === 'Returned' && <><h3>Coordinator feedback</h3><p>{detail.reviewFeedback}</p></>}{detail.status === 'Finalized' && <div className="report-pdf-action"><EvaluationPdfDownload id={detail.id} /></div>}{!student && hasPermission(role, 'evaluations:finalize') && detail.status === 'Submitted' && <div className="modal-actions"><button className="button button-secondary" onClick={() => setAction({ record: detail, kind: 'returned' })}>Return for revision</button><button className="button button-primary" onClick={() => setAction({ record: detail, kind: 'finalized' })}>Finalize evaluation</button></div>}</section>}
    {details && (() => { const record = records.find(item => item.id === details); return record ? <Dialog title="Evaluation details" protectChanges={false} wide onClose={() => setDetails('')}><dl className="info-list"><div><dt>Student</dt><dd>{record.studentName}</dd></div><div><dt>Form</dt><dd>{record.metadata?.formCode ?? record.templateName}</dd></div><div><dt>Status</dt><dd>{record.status}</dd></div><div><dt>Rating period</dt><dd>{record.context.ratingPeriod || 'Not recorded'}</dd></div><div><dt>Rater</dt><dd>{record.evaluatorName}</dd></div></dl><RatingSections criteria={record.criteria} metadata={record.metadata} />{!student && record.evaluatorId === user?.id && ['Draft','Returned'].includes(record.status) && <div className="modal-actions"><button className="button button-primary" onClick={() => { setDetails(''); setEditor(record); }}>Edit evaluation</button></div>}<p className="muted-note">{record.status === 'Finalized' ? 'This official record is finalized and read-only.' : 'Only the authorized evaluator may change this draft.'}</p></Dialog> : null; })()}
