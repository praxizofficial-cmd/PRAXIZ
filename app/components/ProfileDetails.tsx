...
export function ProfileDetails({ role }: { role: RoleId }) {
  ...
  if (role === 'coordinator') identity.push(['Coordinated program',user.scopeProgramName]);
  return <><section className="card profile-identity"><div className="profile-photo-control"><ProfileAvatar name={user.fullName} path={user.avatarPath} /><button className="icon-button" aria-label="Change profile photo" onClick={() => setEditing(true)}><Camera size={20} /></button></div><div><h2>{user.fullName}</h2><p>{roles[role].label}</p><span className="badge badge-info">{user.accountStatus === 'active' ? 'Active account' : 'Pending verification'}</span></div><button className="button button-secondary" onClick={() => setEditing(true)}><Pencil size={17} />Edit profile</button></section>
    {notice && <p role="status" className="form-success">{notice}</p>}
    <section className="card"><h2>Account information</h2><dl className="profile-details-grid">{identity.map(([label,value]) => <div key={label}><dt>{label}</dt><dd>{value || 'Not recorded'}</dd></div>)}</dl><p className="verification-principle"><ShieldCheck size={20} />Contact an administrator to correct verified institutional information.</p></section>
